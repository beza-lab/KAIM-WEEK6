...
def predict():
    ...
    recency_bin = int(data['recency_bin'])
    frequency_bin = int(data['frequency_bin'])
    monetary_bin = int(data['monetary_bin'])
    
    # Convert to numpy array for prediction
    features = np.array([[recency_bin, frequency_bin, monetary_bin]])
    
    # Predict with Logistic Regression
    log_reg_prediction = int(log_reg_model.predict(features)[0])
    log_reg_prob = float(log_reg_model.predict_proba(features)[0][1])
    
    # Predict with Random Forest
    rand_forest_prediction = int(rand_forest_model.predict(features)[0])
    rand_forest_prob = float(rand_forest_model.predict_proba(features)[0][1])
